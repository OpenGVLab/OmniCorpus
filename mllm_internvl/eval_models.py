...
class InternVLChatEvalModel(OpenFlamingoEvalModel):
    def __init__(self, model_args):
        ...
        if self.rank == 0:
            ...
            print(f'[test] image_size: {self.image_size}')
            print(f'[test] template: {self.template}')
    ...
    def get_outputs(
        self,
        batch_text: List[str],
        batch_images: List[List[Tensor]],  # NOTE: this interface is different from that in OpenFlamingoEvalModel (List[List[Image.Image]])
        min_generation_length: int,
        max_generation_length: int,
        num_beams: int,
        length_penalty: float,
        ...
        elif self.tokenizer.convert_tokens_to_ids('<|im_end|>') != 0:  # for InternLM2 (legancy version)
            eos_token = '<|im_end|>'
            eos_token_id = self.tokenizer.convert_tokens_to_ids('<|im_end|>')  # 92542 in InternLM2
        else:
