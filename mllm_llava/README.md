# Pre-training LLaVA with Interleaved Image-text Corpora

This folder contains the implementation of pre-training LLaVA on interleaved image-text corpora, including [OmniCorpus](https://arxiv.org/abs/2406.08418), [MMC4](https://arxiv.org/abs/2304.06939), and [OBELICS](https://arxiv.org/abs/2306.16527).

The codebase is based on [LLaVA](https://github.com/haotian-liu/LLaVA), Thanks for their great work.
...
conda create -n llava_interleaved python=3.10 -y
conda activate llava_interleaved

# install package
pip install --upgrade pip  # enable PEP 660 support
pip install torch==2.1.2 torchvision==0.16.2 torchaudio==2.1.2 --index-url https://download.pytorch.org/whl/cu118  # we use a different cuda version, remember to set corresponding CUDA runtime version.
pip install -e .
...
For the SFT and evaluation data preparation, please refer to [data doc](https://github.com/haotian-liu/LLaVA/blob/main/docs/Data.md) and [evaluation](https://github.com/haotian-liu/LLaVA/blob/main/docs/Evaluation.md) doc of LLaVA.

For few-shot SFT setting, a [preparation script](./scripts/convert_fewshot_sft.py) is presented. We also upload [the annotation file](https://huggingface.co/datasets/Qingyun/llava_v1_5_fewshot_sft) to Huggingface. We construct few-shot fine-tuning by compose multiple examples into a few-shot sample with multiple multimodal conversations. We also try filtering out the single-shot samples that have been selected for few-shot samples to avoid repetition. TextCap was excluded from the construction of few-shot samples due to its observed impact on performance. We provide a [gradio app](scripts/vis_fewshot_sft.py) to visualize the few-shot sft data.
...
```
torchrun --nnodes=${N_NODES} --nproc-per-node=8 \
    --master_port 12955 --master_addr ${MASTER_ADDR} --node_rank ${THE_ID} \
    llava/train/train_interleaved_mem.py \
    --deepspeed ./scripts/zero3.json \
    --model_name_or_path lmsys/vicuna-7b-v1.5 \
    --version plain \
    --data_type "obelisc" \
    --data_path "sampled_omnicorpus_cc.json" \
    --image_folder "s3://omnicorpus/raw-images/" \
    --vision_tower openai/clip-vit-large-patch14-336 \
    --pretrain_mm_mlp_adapter ./checkpoints/$PRETRAIN_RUN_NAME/mm_projector.bin \
...
### Pre-training with interleaved image-text corpus with retrieval arrangement
...
torchrun --nnodes=${N_NODES} --nproc-per-node=8 \
    ...
    --version plain \
    --data_type "new_mmc4" \
    --data_path "sampled_omnicorpus_cc_with_similarities.json" \
    --image_folder "s3://omnicorpus/raw-images/" \
    --vision_tower openai/clip-vit-large-patch14-336 \
    --pretrain_mm_mlp_adapter ./checkpoints/$PRETRAIN_RUN_NAME/mm_projector.bin \
    --mm_projector_type mlp2x_gelu \
...
### Pre-training based on InternLM2-Chat

```
# NOTE use `--version internlm2` when tuning
torchrun --nnodes=${N_NODES} --nproc-per-node=8 \
    --master_port 12955 --master_addr ${MASTER_ADDR} --node_rank ${THE_ID} \
    llava/train/train_interleaved_mem.py \
    --deepspeed ./scripts/zero3.json \
...
### SFT interleaved pretrained model with few-shot setting
...
torchrun --nnodes=${N_NODES} --nproc-per-node=8 \
    ...
    --data_type "incontext_sft" \
    --data_path "./playground/data/llava_v1_5_filtered_mix355k.json" \
    --image_folder "./playground/data/llava_v1_5_5shots_mix260k.json" \
    --incontext_data_path "./playground/data/" \
    --incontext_image_path "./playground/data" \
    --vision_tower openai/clip-vit-large-patch14-336 \
    --mm_projector_type mlp2x_gelu \
    --mm_vision_select_layer -2 \
    --mm_use_im_start_end False \
...
The few-shot evaluation setting is implemented in [this vqa script](llava/eval/model_vqa_loader_few_shot.py) and [this caption script](llava/eval/model_captioning_loader_few_shot.py).

### Evaluate Caption (coco or flickr)

```
python -m llava.eval.model_captioning_loader_few_shot \
    --model-path $MODEL_PATH \
    --answers_file ./playground/data/eval/caption/answers_${SHOTS}shots/${RUN_NAME}_${DATASET_NAME}.jsonl \
    --results_path ./playground/data/eval/caption/answers_${SHOTS}shots/${RUN_NAME}_${DATASET_NAME}.tmp.json \
    --temperature 0 \
    --conv-mode ${CONV_MODE} \
    --shots $SHOTS \
    --seed 42 \
    ...
    --query_set_size 2048 \
    --rices True \
    --cached_demonstration_features ${RICES_FEATURES_PATH} \
    --coco_train_image_dir_path "${COCO_ROOT}/train2014" \
    --coco_val_image_dir_path "${COCO_ROOT}/val2014" \
    --coco_annotations_json_path "${COCO_ROOT}/annotations/captions_val2014.json" \
    --coco_karpathy_json_path "${ANN_ROOT}/dataset_coco.json" \
    --flickr_image_dir_path "${FLICKR_ROOT}/flickr30k-images" \
    --flickr_karpathy_json_path "${ANN_ROOT}/dataset_flickr30k.json" \
    --flickr_annotations_json_path "${ANN_ROOT}/flickr30k_dataset_flickr30k_coco_style.json" \
    --dataset_name ${DATASET_NAME}
```

### Evaluate VQA (vqav2, okvqa, textvqa, vizwiz)

```
# take okvqa as an example
python -m llava.eval.model_vqa_loader_few_shot \
    --model-path ${MODEL_PATH} \
    --question-file ./playground/data/eval/okvqa/llava_okvqa_mscoco_val2014.jsonl \
    --image-folder ./playground/data/eval/okvqa/val2014 \
    --answers-file ./playground/data/eval/okvqa/${ANSWERS_FOLDER}/${RUN_NAME}.jsonl \
    --temperature 0 \
    --conv-mode ${CONV_MODE} \
    --shots ${SHOTS} \
    --query_set_size 2048 \
    --rices True --seed 42 --deterministic True \
    --cached_demonstration_features path_to_cached_rices_features \
    --train_image_dir_path "${COCO_ROOT}/train2014" \
